...
class Trainer:
    ...
    def save_checkpoint(self):
        ...
        logger.info(f"saving {self.config.ckpt_path + str(self.epoch)}")
        self.epoch += 1
        torch.save(raw_model.state_dict(), self.config.ckpt_path)
        if self.config.generate:
            prompt = self.train_dataset.__getitem__(0)[0]
            self.generate_sample(prompt, 100)

    def generate_sample(self, prompt, max_len, temperature=1.0):
        prompt.to(self.device)
        y = sample(self.model, prompt, max_len, temperature=temperature,sample=True, top_k=40)[0]
